...
final class Node {
    ...
    var myAccount: Account {
        let account = Node.shared.getMyAccount(completion: <#T##(Account?, NodeError?) -> Void#>)
    }
        ...
        getMyAccount { [weak self] (account, error) in
            ...
            do {
                ...
                self?.saveSync([genesisBlock]) { (error) in
                    ...
                    do {
                        let lightBlock = try LightBlock(data: secondBlock)
    ...
    func parsePacket(_ packet: Packet) {
        ...
        if let blocks = packet.blocks, blocks.count > 1 {
            Node.shared.localStorage.getLatestBlock { [weak self] (block: FullBlock?, error: NodeError?) in
                ...
                if let block = block {
                    ...
                    /// Since no blocks exists, a new account has to be created. This point is reached due to a catastrphic failure.  The balance will indicate 0.
