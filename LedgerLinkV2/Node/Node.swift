...
final class Node {
    ...
    private var validatedAccounts: [TreeConfigurableAccount] = [] /// updated accounts after the transfer to be added to the block
    private var unvalidatedBlocks: Multiset<FullBlock> = [] /// receive blocks from peers and blocks generated by your own device prior to being added to the blockchain. Multiset keeps tally of each unique blocks.
    ...
    /// 1. Process the last block by choosing one from the pool.  
    func createBlock(completion: @escaping (LightBlock) -> Void) {
        Deferred {
            /// Select the majority block
        ...
        .flatMap({ (_) -> AnyPublisher<Bool, NodeError> in
        ...
        .sink { completion in
            ...
            self?.addUnvalidatedBlock(block)
